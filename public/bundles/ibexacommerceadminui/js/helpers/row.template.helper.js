export default class RowTemplateGenerator {
    ...
    mostSearch(row) {
        ...
        rowTemplate = rowTemplate.replace('{{ LOG_MESSAGE }}', window.ibexa.helpers.text.escapeHTML(row.logMessage || row[0]));
    ...
    lastSearch(row) {
        ...
        rowTemplate = rowTemplate.replace('{{ LOG_MESSAGE }}', window.ibexa.helpers.text.escapeHTML(row.logMessage || row[1]));
    ...
    dashboard(row) {
        ...
        rowTemplate = rowTemplate.replace('{{ SKU }}', window.ibexa.helpers.text.escapeHTML(row.sku || row[0]));
        rowTemplate = rowTemplate.replace('{{ COUNT_SKU }}', row.countSku || row[1]);
        rowTemplate = rowTemplate.replace('{{ NAME }}', window.ibexa.helpers.text.escapeHTML(row.name || row[2]));
    ...
    lastOrders(row) {
        ...
        rowTemplate = rowTemplate.replace('{{ BUYER }}', window.ibexa.helpers.text.escapeHTML(row.buyer || row[1] || ''));
    ...
    bestClients(row) {
        ...
        rowTemplate = rowTemplate.replace('{{ NAME }}', window.ibexa.helpers.text.escapeHTML(row.name || row[0] || ''));
        rowTemplate = rowTemplate.replace('{{ AMOUNT }}', row.amount || row[1]);
