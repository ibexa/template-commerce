...
export default class Cart {
    constructor(options = {}) {
        ...
        const anonymousUserId = parseInt(document.querySelector('meta[name="AnonymousUserId"]').content, 10);

        if (this.userId === anonymousUserId) {
            this.authorize().then(() => this.loadCart());
        } else {
            this.loadCart();
        }
    }
    ...
    authorize() {
        return cartService.authorize().then((response) => {
            document.querySelector('meta[name="CSRF-Token"]').content = response.Token.value;
        });
    }

    createCart() {
        return cartService.createCart(this.currencyCode).then((response) => {
    ...
    loadCart() {
        return cartService.loadUserCarts(this.userId).then((response) => {
    ...
    addProduct(productCode, quantity, context = {}) {
        ...
        return cartService.createCartEntry(this.getIdentifier(), productCode, quantity, context).then((response) => {
    ...
    updateEntryQuantity(entryIdentifier, newQuantity, context = {}) {
        ...
        return cartService.updateProductQuantity(this.getIdentifier(), entryIdentifier, newQuantity, context).then((response) => {
            return cartService.loadCartSummary(response.Cart.identifier).then((summaryResponse) => {
                this.cartData = response.Cart;
                this.cartSummary = summaryResponse.CartSummary;
                this.onCartDataChanged();
