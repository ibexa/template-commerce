...
export default class Maincart {
    ...
    attachItemEventListeners(entry, itemNode) {
        ...
        guardQuantityInputValue(quantityInput, () => {
            const currentEntry = this.cart.getEntryByIdentifier(entry.identifier);

            this.getMaxQuantity(currentEntry);
        });

        removeItemBtn?.addEventListener(
            'click',
            (event) => {
                const currentEntry = this.cart.getEntryByIdentifier(entry.identifier);

                this.handleItemRemove(event, currentEntry);
            },
            false,
        );
        quantityInput?.addEventListener(
            ...
            (event) => {
                ...
                this.handleQuantityInputChangeDebounced(event, currentEntry);
            },
            false,
        );
        quantityInput?.addEventListener(
